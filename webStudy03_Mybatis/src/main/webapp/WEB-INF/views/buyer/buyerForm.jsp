...
<script type="text/javascript">
	...
	<form method="post" id="buyerForm">
		<table class="table">
			<tbody>
				 <tr>
					<th>거래처 ID</th>
					<td><input type="text" name="buyerId" required value="${buyer.buyerId}" />
					<label id="buyerId-error" class="error"
						for="buyerId">${errors.buyerId}</label></td>
				</tr> 
				<tr>
					<th>거래처 이름</th>
					<td><input type="text" name="buyerName" required value="${buyer.buyerName}" /><label id="buyerName-error"
				...
				<tr>
					...
					<select name="buyerLgu" required >
						...
						<c:forEach items="${lprodList }" var="lprod">
							<option class="${lprod.lprod_gu }" value="${lprod.lprod_gu }" >
								${lprod.lprod_nm }
							</option>
						</c:forEach>
					</select>
			...
			<th>거래처 거래 상태</th>
			<td>
			<select name="buyerStatus" required >
				<option value>상태</option>
				<c:forEach  items="${statusList}" var="statusType">	
						<option value="${statusType.statusName }">${statusType.statusName }</option>
				</c:forEach>
			</select>
			 <label id="buyerStatus-error" class="error" for="buyerStatus">${errors.buyerStatus}</label>
				</td>
		</tr>
				<tr>
					<th>거래처 은행</th>
					<td>
					<input type="text" name="buyerBank" value="${buyer.buyerBank}" />
					
					<label id="buyerBank-error" class="error" for="buyerBank">${errors.buyerBank}</label></td>
				</tr>
				<tr>
					<th>거래처 계좌번호</th>
					<td><input type="text" name="buyerBankno"  value="${buyer.buyerBankno}" /><label id="buyerBankno-error"
						class="error" for="buyerBankno">${errors.buyerBankno}</label></td>
				</tr>
				<tr>
					<th>거래처 계좌 명의</th>
					<td><input type="text" name="buyerBankname"  value="${buyer.buyerBankname}" /><label id="buyerBankname-error"
						class="error" for="buyerBankname">${errors.buyerBankname}</label></td>
				</tr>
				<tr>
					<th>거래처 우편 번호</th>
					<td><input type="text" name="buyerZip" value="${buyer.buyerZip}" /><label id="buyerZip-error"
				...
				<tr>
					<th>거래처 메일</th>
					<td><input type="text" name="buyerMail" required  value="${buyer.buyerMail}" />
					<label id="buyerMail-error" class="error" for="buyerMail">${errors.buyerMail}</label></td>
				</tr>
				<tr>
					<th>거래처 책임자</th>
					<td><input type="text" name="buyerCharger" value="${buyer.buyerCharger}" />
	...
	$(function(){
		...
		$("select[name='buyerLgu']").val("${buyer.buyerLgu }");
		$("select[name='buyerStatus']").val("${buyer.buyerStatus }");
